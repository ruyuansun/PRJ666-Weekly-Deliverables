...
function product_add_post(router) {
    router.post("/addProd", crypto.authorize_token, (req, res) => {
        ...
        let { product_name, price, location } = req.body;

        const query = "INSERT INTO products (uid, product_name, price, location) VALUES (?, ?, ?, ?)"
        db.query(query, [token_data.uid, product_name, price, location ])
        .then((result) => {
            if (result) {
